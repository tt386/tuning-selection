...
parser.add_argument('-a','--aspectratio',type=float,required=True,
        help='h/a')
args = parser.parse_args()

a = float(args.aspectratio) 

field_width = np.sqrt(A/a)
field_height = np.sqrt(a*A)
...
if not os.path.isdir(NumSaveDirName):
    ...
    print("Created Directory for aspect ratio",a)
...
# Parameters
##############################################################################
L = field_width + 10.0   # your padding choice

if field_width >= 0.03:#0.1:#5:
    dx = 0.005# halved it
#dx = 0.01                # fixed spatial resolution

SystemHeight = field_height+10#10

nx = int(L / dx)
ny = int(SystemHeight/dx)
...
n_field_height = int(np.round(field_height/dx))
...
half_n = n_field // 2
half_height = n_field_height // 2

field_mask = np.zeros((ny, nx), dtype=bool)
field_mask[iy0-half_height:iy0+half_height+1, ix0-half_n:ix0+half_n+1] = True
...
y_int = np.linspace(0, SystemHeight, ny)
Change = simps(simps(Resistant_End - Resistant_Start, x_int), y_int)
