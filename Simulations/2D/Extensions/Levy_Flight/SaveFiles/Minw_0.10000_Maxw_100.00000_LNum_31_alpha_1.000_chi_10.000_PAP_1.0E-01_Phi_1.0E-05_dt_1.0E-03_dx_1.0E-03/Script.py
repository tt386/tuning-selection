...
# -----------------------------
# FFT split-step phase 1 solver (with Lévy flights)
# -----------------------------
def phase1_fft_absorb(u0, dx, D, PAP, field_mask,
                      dt=0.05, chi=1.0, alpha=1.0):
    """
    Phase 1: periodic diffusion + Lévy flights + perfect killing in field.

    u0        : (ny, nx) array at t = 0
    dx        : grid spacing
    D         : normal diffusion coefficient
    PAP       : end time of pesticide phase
    field_mask: boolean array True inside field
    dt        : time step (can be reasonably large)
    chi       : strength of Lévy component
    alpha     : Lévy exponent in (0, 2]; alpha=2 → pure diffusion
    """
    ny, nx = u0.shape
    u = u0.copy()
    ...
    # |k|^alpha term for Lévy flights
    k_abs_alpha = (k2 + 1e-30)**(alpha / 2.0)  # epsilon avoids 0**0 if alpha→0

    t = 0.0
    while t < PAP - 1e-12:
        dt_step = min(dt, PAP - t)

        # diffusion + Lévy substep (exact in Fourier)
        u_hat = np.fft.fft2(u)
        u_hat *= np.exp(-(D * k2 + chi * k_abs_alpha) * dt_step)
        u = np.real(np.fft.ifft2(u_hat))

        # killing substep (perfect death in the field)
        u[field_mask] = 0.0
...
##############################################################################
# PHASE 2: PAP < t < 1
# free periodic diffusion (+ Lévy) in one go
##############################################################################
tau = T - PAP
...
KX, KY = np.meshgrid(kx, ky)
k2 = KX**2 + KY**2
k_abs_alpha = (k2 + 1e-30)**(alpha / 2.0)   # same alpha as in phase 1

u_hat = np.fft.fft2(u_at_PAP)
propagator = np.exp(-(D * k2 + chi * k_abs_alpha) * tau)
