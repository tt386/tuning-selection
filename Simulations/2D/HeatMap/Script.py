...
parser.add_argument('-w','--width',type=float,required=True,
        help='width of Selection region')

parser.add_argument('-H','--height',type=float,required=True,
        help='height of Selection region')

args = parser.parse_args()

field_width = float(args.width)
field_height = float(args.height)

NumSaveDirName = (SaveDirName +
    "/width_%0.5f"%(field_width) +
    "_height_%0.5f"%(field_height))
...
# Parameters
##############################################################################
L = field_width + 10#10.0   # your padding choice

if field_width > 0.5:
    dx = 0.01
#dx = 0.01                # fixed spatial resolution

SystemHeight = field_height+10
