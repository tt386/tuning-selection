...
if __name__ == "__main__":
    ...
    Single_w_Plot(directory,True)
"""
def Single_w_Plot(directory,PlotAll = False):
    ...
    smoothgrad = savgol_filter(GradientLogdRList,51,3)

    plt.figure()
    plt.plot(GradientLogLList,GradientLogdRList)
    plt.plot(GradientLogLList,smoothgrad)
    plt.savefig(str(directory) + "/dR_Grad.png",bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(str(directory) + "/dR_Curv.png",bbox_inches='tight')
    plt.close()
    ...
    #plt.text(-1,-1,r"$\omega=%0.1f$"%w,fontsize=50,fontname="Arial")
    ...
    #Formatting
    ax.set_xticks([-1,MinGradient,MindR,MinCurvature,3])
    ax.set_xticklabels(
        [r'$10^{-1}$',r'$K_{L}$',r'$K^{*}$',r'$K_{U}$',r'$10^3$'])
    ...
    #Saving
    plt.savefig(str(directory) + '/dR_Detail.png',bbox_inches='tight')
    plt.savefig(str(directory) + '/dR_Detail.eps',bbox_inches='tight')
    ...
    return(MinGradient,MindR,MinCurvature,Phi,PAP,dRList,LList)
