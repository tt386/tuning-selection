...
#Multiple
M_xlist, M_PAPDist = Core.PAPDist_Multiple_ConstrainedFinite(B,D,N,xbound,dx,Phi,PAP)#Core.PAPDist_Single(xbound,L,dx,Phi,PAP)
Kernel = Core.Kernel(PAP,M_xlist)
M_EndDist = Core.EndDist_Single(M_PAPDist,Kernel,M_xlist,D,dx)
M_ApproxIntegral = Core.dR(Phi,M_EndDist,M_xlist)
print(N,"Finished Multiple")

#Isolated Approx
I_xlist,I_PAPDist = Core.PAPDist_Single(xbound,D*B/N,dx,Phi,PAP)
Kernel = Core.Kernel(PAP,I_xlist)
I_EndDist = Core.EndDist_Single(I_PAPDist,Kernel,I_xlist,D*B/N,dx)
I_ApproxIntegral = N*Core.dR(Phi,I_EndDist,I_xlist)
print(N,"Finished Isolated")

#Large Single Approx
I_TOT_xlist,I_TOT_PAPDist = Core.PAPDist_Single(xbound,D,dx,Phi,PAP)
Kernel = Core.Kernel(PAP,I_TOT_xlist)
I_TOT_EndDist = Core.EndDist_Single(I_TOT_PAPDist,Kernel,I_TOT_xlist,D,dx)
I_TOT_ApproxIntegral = Core.dR(Phi,I_TOT_EndDist,I_TOT_xlist)
print(N,"Finished Isolated Total")


#Periodic Approx
K = D*B#D*B/N + D*(1-B)/(N-1)
w = D*(1-B)#D*(1-B)/(N-1) / K
if N > 1:
    K = D*B/N + D*(1-B)/(N-1)
    w = D*(1-B)/(N-1) / K
    ...
    P_xlist,P_PAPDist,P_EndDist,P_ApproxIntegral = I_xlist,I_PAPDist,I_EndDist,I_ApproxIntegral
